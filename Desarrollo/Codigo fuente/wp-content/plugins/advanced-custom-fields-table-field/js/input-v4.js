	...
	function ACFTableField() {
			...
			// BODY ROWS {
				...
				if ( p.data.b ) {
					...
					for ( i in p.data.b) {
						...
						p.obj_bottom_row.before( t.param.htmltable.body_row.replace( '<!--ph-->', "Hotel "+ ( parseInt(i) + 1) ) ); // parseInt(i) + 1
					}
				}
		...
		t.table_top_labels = function( p ) {
			...
			p.obj_table.find( '.acf-table-top-cont' ).each( function() {
				...
				if ( string === "A") {
					...
					string = "Habitacion doble";
				} else if( string === "C"){
					string = "Habitacion triple";
				}
				$( this ).text( string );
		...
		t.table_left_labels = function( p ) {
			...
			p.obj_table.find( '.acf-table-body-left' ).each( function() {
				...
				$( this ).find( '.acf-table-body-cont' ).text( "Hotel " + i );
